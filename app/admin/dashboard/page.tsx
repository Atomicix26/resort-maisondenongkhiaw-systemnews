...
import {
  CalendarDays, Users, Star, BedDouble, CheckCircle2, Clock,
...
// ── Types ────────────────────────────────────────────────────────
interface Stats {
  rooms:    { total: number; available: number; occupied: number; maintenance: number; reserved: number }
  bookings: { total: number; pending: number; confirmed: number; checkedIn: number; checkedOut: number; completed: number; cancelled: number; noShow: number; todayCheckIns: number; todayCheckOuts: number }
  ...
  const inner = (
    <div className={`bg-white rounded-xl border p-5 shadow-sm hover:shadow-md transition-all
      ...
      <div className="flex items-start justify-between">
        ...
        {href && !alert && <ArrowUpRight size={14} className="text-gray-400" />}
      </div>
...
export default function AdminDashboard() {
  ...
  // /booking เป็นหน้าจัดการห้องของ SUPERADMIN เท่านั้น — ADMIN ไม่ควรเห็นลิงก์ที่กดแล้วโดน 403 (BUG-018)
  const roomHref = session?.user?.role === "SUPERADMIN" ? "/booking" : undefined
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-7 gap-3 mb-8">
          {[
            ...
            <div key={label} className={`${bg} ${border} border rounded-xl p-4 text-center`}>
              ...
              <p className={`text-[11px] font-semibold ${text} mt-0.5 uppercase tracking-wide`}>{label}</p>
            </div>
          ))}
        </div>
                  ...
                  return (
                    <tr key={b.id} className="hover:bg-gray-50 transition-colors">
                      <td className="px-5 py-3">
                        ...
                        <p className="text-gray-500 text-[11px]">{b.user.email}</p>
                      </td>
