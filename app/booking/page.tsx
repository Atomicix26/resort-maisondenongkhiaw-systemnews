...
// ✅ ເພີ່ມ path ທຸກລາຍການ
const navItems = [
  { icon: BedDouble,      label: "ຈັດການຂໍ້ມູນຫ້ອງ",    path: "/booking",  hasDropdown: false },
  { icon: CalendarCheck2, label: "ຈັດການຂໍ້ມູນພະນັກງານ", path: "/staff",    hasDropdown: false },
  { icon: CalendarDays,   label: "ຈັດການການຈອງ",         path: "/schedule", hasDropdown: false },
  { icon: BarChart3,      label: "ຈັດການສິວິວ",           path: "/review",   hasDropdown: false },
...
export default function AdminDashboard() {
  ...
  const [activeNav, setActiveNav]       = useState(0); // booking = index 0
            ...
            <div className="divide-y divide-gray-50">
              {rows.map((row) => {
                ...
                return (
                  ...
                  >
                    {/* ID */}
                    <div className="w-16 text-[12px] text-gray-400 font-mono">
                      {row.id.padStart(2, "0")}
                    </div>
                      ...
                      {isEditing ? (
                        <div className="relative">
                          ...
                          <input
                            ...
                            className="w-full pl-6 pr-3 py-1.5 bg-white border border-blue-300 rounded-md text-[13px] outline-none focus:ring-2 focus:ring-blue-100"
                          />
                        </div>
                      ) : (
                    ...
                    <div className="w-36">
                      {isEditing ? (
                        <select
                          ...
                          className="px-2 py-1.5 bg-white border border-blue-300 rounded-md text-[12px] outline-none cursor-pointer font-medium"
                        >
                          {statusOptions.map((s) => (
                          ...
                          {openDropdown === row.id && (
                            <div className="absolute right-0 mt-1 w-32 bg-white rounded-lg shadow-lg border border-gray-100 py-1 z-50">
                              <button
                                ...
                                className="w-full text-left px-3 py-2 text-[12px] text-gray-600 hover:bg-blue-50 hover:text-blue-600"
                              >
                                ✏️ ແກ້ໄຂ
                              </button>
