...
export default function PaymentContent() {
  ...
  useEffect(() => {
    ...
    fetch("/api/rooms")
      .then((r) => r.json())
      .then((data) => {
        const found = (Array.isArray(data) ? data : []).find((r: Room) => r.id === roomId)
        setRoom(found ?? null)
      })
      .catch(() => setRoom(null))
  ...
  // ✅ revoke Object URL เมื่อ preview เปลี่ยน — ป้องกัน memory leak
  useEffect(() => {
  ...
  async function handleConfirm() {
    ...
    try {
      const res  = await fetch("/api/bookings", {
        ...
        // ✅ ไม่ส่ง totalPrice — server คำนวณเอง
        body: JSON.stringify({ roomId, checkIn, checkOut, guests: guests.toString(), specialRequest: special }),
      })
      ...
      <div className="min-h-screen flex items-center justify-center bg-gray-50 font-lao px-4">
        <div className="bg-white rounded-2xl shadow-xl p-10 max-w-md w-full text-center">
          ...
          <p className="text-[13px] text-gray-500 mb-1">
            {method === "transfer" ? "ກຳລັງລໍຖ້າ Admin ກວດສອບ" : "ການຈອງຢືນຢັນແລ້ວ"}
          </p>
          ...
          <div className="bg-gray-50 rounded-xl p-4 my-5 text-left space-y-2 text-[12px]">
            <div className="flex justify-between"><span className="text-gray-400">ຫ້ອງ</span><span className="font-medium">{room.name}</span></div>
            ...
            <div className="flex justify-between"><span className="text-gray-400">ລວມທັງໝົດ</span><span className="font-bold text-blue-600">{fmt(total)} ₭</span></div>
          </div>
          <div className="flex gap-3">
            <button onClick={() => router.push("/history")} className="flex-1 py-2.5 border border-gray-200 rounded-xl text-[13px] text-gray-600 hover:bg-gray-50 transition-all">ປະຫວັດການຈອງ</button>
            <button onClick={() => router.push("/profile")} className="flex-1 py-2.5 bg-blue-600 hover:bg-blue-700 text-white rounded-xl text-[13px] font-semibold transition-all">ໜ້າຫຼັກ</button>
          </div>
        </div>
      </div>
    )
      ...
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-4xl overflow-hidden flex flex-col md:flex-row">
        ...
        <div className="flex-[1.3] p-8 md:p-12">
          <button onClick={() => step === "pay" ? setStep("confirm") : router.back()}
            ...
            <ArrowLeft size={14} />{step === "pay" ? "ກັບໄປຢືນຢັນ" : "ກັບຄືນ"}
          </button>

          <div className="flex items-center gap-2 mb-8">
            {(["confirm", "pay"] as const).map((s, i) => (
              <div key={s} className="flex items-center gap-2">
                <div className={`w-6 h-6 rounded-full flex items-center justify-center text-[11px] font-bold transition-colors ${step === s ? "bg-blue-600 text-white" : (step === "pay" && s === "confirm") ? "bg-green-500 text-white" : "bg-gray-100 text-gray-400"}`}>
                  {step === "pay" && s === "confirm" ? <CheckCircle2 size={13}/> : i+1}
                </div>
                <span className={`text-[12px] font-medium ${step === s ? "text-gray-900" : "text-gray-400"}`}>{s === "confirm" ? "ຢືນຢັນຂໍ້ມູນ" : "ຊຳລະເງິນ"}</span>
            ...
            <div className="space-y-5">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="bg-[#E9F0F6] rounded-lg p-3">
                  ...
                  <p className="text-[13px] font-semibold text-gray-800 flex items-center gap-1.5"><Calendar size={13} className="text-blue-500"/>{checkIn}</p>
                </div>
                <div className="bg-[#E9F0F6] rounded-lg p-3">
                  <p className="text-[10px] text-gray-400 uppercase tracking-wide mb-1">ວັນທີເຊັກເອົາ</p>
                  <p className="text-[13px] font-semibold text-gray-800 flex items-center gap-1.5"><Calendar size={13} className="text-blue-500"/>{checkOut}</p>
                </div>
              </div>
              <div>
                ...
                <div className="relative">
                  <select value={guests} onChange={(e) => setGuests(Number(e.target.value))}
                    ...
                    {Array.from({ length: room.capacity }, (_, i) => i+1).map((n) => <option key={n} value={n}>{n} ທ່ານ</option>)}
                  </select>
                  <ChevronDown size={14} className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-500 pointer-events-none"/>
                </div>
              </div>
              <div>
                <p className="text-[11px] font-semibold text-gray-400 uppercase tracking-wider mb-1">ຄຳຮ້ອງຂໍພິເສດ</p>
                <textarea value={special} onChange={(e) => setSpecial(e.target.value)} rows={2}
                  placeholder="ຕ້ອງການອາຫານພິເສດ..." className="w-full bg-[#E9F0F6] rounded-lg px-4 py-3 text-[13px] outline-none resize-none text-gray-800"/>
              </div>
            ...
            <div className="space-y-5">
              <div>
                ...
                <div className="grid grid-cols-2 gap-3">
                  <button onClick={() => setMethod("transfer")} className={`flex items-center gap-2.5 p-3.5 rounded-xl border-2 text-[13px] font-medium transition-all ${method === "transfer" ? "border-blue-500 bg-blue-50 text-blue-700" : "border-gray-200 text-gray-600 hover:border-gray-300"}`}>
                    <Banknote size={18}/> ໂອນເງິນ
                  </button>
                  <button onClick={() => setMethod("credit_card")} className={`flex items-center gap-2.5 p-3.5 rounded-xl border-2 text-[13px] font-medium transition-all ${method === "credit_card" ? "border-blue-500 bg-blue-50 text-blue-700" : "border-gray-200 text-gray-600 hover:border-gray-300"}`}>
                  ...
                  <div onClick={() => fileRef.current?.click()}
                    ...
                    {slipPreview ? (
                      ...
                      <>
                        ...
                        <p className="text-[10px] text-gray-300 mt-1">PNG, JPG, WEBP — ສູງສຸດ 5MB</p>
                      </>
                    )}
                  </div>
                </div>
        ...
        <div className="flex-1 bg-[#F8FAFC] border-l border-gray-100 p-8 md:p-10 flex flex-col justify-between">
          ...
          <div className="space-y-3 pt-5 border-t border-gray-200">
            <div className="flex justify-between text-[12px]"><span className="text-gray-400">ລາຄາ / ມື້</span><span className="font-medium text-gray-800">{fmt(room.price)} ₭</span></div>
            <div className="flex justify-between text-[12px]"><span className="text-gray-400">ຈຳນວນວັນ</span><span className="font-medium text-gray-800">{days > 0 ? `${days} ມື້` : "-"}</span></div>
            <div className="flex justify-between text-[12px]"><span className="text-gray-400">ຈຳນວນຄົນ</span><span className="font-medium text-gray-800">{guests} ທ່ານ</span></div>
            <div className="flex justify-between items-center pt-3 border-t border-gray-200">
              <span className="text-[13px] font-bold text-gray-900">ລວມທັງໝົດ</span>
              <span className="text-[22px] font-black text-blue-600">{days > 0 ? `${fmt(total)} ₭` : "-"}</span>
            </div>
          </div>
        </div>
      </div>
